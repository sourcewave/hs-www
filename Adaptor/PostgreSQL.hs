...
import Data.List (foldl1, foldl', find)
...
import Control.Monad (forM, liftM, forever)
...
getUntil c = fmap (B.toStrict  . BB.toLazyByteString) (getUntil' c mempty)
  where getUntil' c a = do
          n <- getWord8
          if n == c then return a else getUntil' c (a `mappend` BB.word8 n)
...
instance Binary PgMessage where
  get = do
    ...
    len <- getInt32 
    case b of 
      'R' -> fmap Authentication getInt32
      'S' -> do 
...
-}
  ...
  put (Password s) = putByte 'p' >> putStringMessage s
