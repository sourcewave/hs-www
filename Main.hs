...
getVursionFromSession (SessionContext a) = let uid:cmp:rol:vurs:intercom:reasons:paying:_ = a in vurs
  ...
  case sess of
    ...
    _ -> if noUser sess && needsLogin uu then sendRedirect cgir "/login/" else
      if not (noUser sess) && (null uu || uu == "index.html") then
        let SessionContext (_:_:rol:_:_:_:_) = sess
...
instance Show SessionContext where
  show (SessionContext a) = let uid:cmp:rol:vurs:intercom:reasons:paying:_ = a in
    if B.null uid then ""
    else (B.unpack . B.concat) ["<script>document.sessionState={'userid': " ,enstr uid ,
                            ",'company': " , enstr cmp , ",'role':", enstr rol ,",'intercom':",enstr intercom, ",'reasons':",enstr reasons, ",'paying':", enstr paying, "};</script>"]
